...
#  JGM         21.0      2021/08/28    Mods for Terse Report (-q) for dealer (None yet)
...
sub cleanup_DDSHand {        # one hand 16 or so chars
   ...
   local $req_dbg = $req_dbg + 1 ;
   $_ = $DDShand ;
   print STDERR "cleanup_DDSHand::Input  [$DDShand] {$Debug ge $req_dbg }\n" if $Debug >= $req_dbg;
   chomp;
   $_ = uc $_ ;
   if (~m/^(\s*[NESW][ :])(.+)/) { $_ = $2 ; }
   if ( m/^\s*([0-9AKQJTakqjt .:]{13,})/ )    { $DDS_str = $1 ; print }
   elsif ( m/^\s*#/) { next ; }  # Skip comments
   else { return $DDSHand ; }  # Hand was not in DDS format.
   print STDERR "cleanup_DDSHand:: Matched [$DDS_str] {$Debug ge $req_dbg }\n" if $Debug >= $req_dbg;
   ...
   $hand = join ',',@suits; #commas are needed for further processing..
      print STDERR "cleanup_DDSHand:: Hand[$hand] { $Debug ge $req_dbg} +1\n" if $Debug >= $req_dbg+1;
   &show_arr( "cleanup_DDSHand:: Suits: ",  \@suits) if $Debug >= $req_dbg+1;
   return $hand;
 }
...
sub split_path {
   ...
   print STDERR "[$1] , [$2] , [$3] $Debug ge $req_dbg \n" if $Debug >= $req_dbg;
...
sub Init_hand_hashes {     # define the hand keys and give them defaults. All except SUIT ptrs set at compile time.
   ...
   my $key_t ;
   foreach $hnd_href (@_) {
      print STDERR "Init_hand_hashes:: Starting copy default to $hnd_href->{NAME} {$Debug ge $req_dbg}\n" if $Debug >= $req_dbg;
...
sub show_hash_key_types {
   my $href = shift ;
   my $hash_t = ref $href ;
   print STDERR "In hash of type $hash_t \n";
   foreach $key (sort keys %{$href} ) {
      $key_t = ref($href->{$key} );
      if ( $key_t ) { printf STDERR "Key[$key] has type [$key_t] \n" ;}
      else          { printf STDERR "Key[$key] has type [SCALAR] \n" ;}
   }
   return 1;
...
sub check_setup {
      ...
      &show_DOP_hash ("Init Deal",    \%Deal );       print STDERR "================ o-o ==============\n";
      &show_DOP_hash ("Init Deal_NS", \%Deal_NS );    print STDERR "================ o-o ==============\n";
      &show_DOP_hash ("Init Hand_W " , \%Hand_W );    print STDERR "================ o-o ==============\n";
      &show_DOP_hash ("Init Hand_E " , \%Hand_E );    print STDERR "================ o-o ==============\n";
      &show_DOP_hash ("Init Hand_S " , \%Hand_S );    print STDERR "================ o-o ==============\n";
      &show_DOP_hash ("Init Hand_N " , \%Hand_N );    print STDERR "================ o-o ==============\n";
      &show_DOP_hash ("Init Suit N_Spades " , \%Spades_N );    print STDERR "================ o-o ==============\n";
      &show_DOP_hash ("Init Suit S_Hearts " , \%Hearts_S );    print STDERR "================ o-o ==============\n";
      &show_DOP_hash ("Init Suit W_Diams " ,  \%Diams_W  );    print STDERR "================ o-o ==============\n";
      &show_DOP_hash ("Init Suit E_Clubs " ,  \%Clubs_E  );    print STDERR "================ o-o ==============\n";
...
sub DOP2Results {  #needs only the two hashes ; GIB2Results could fill in the Side.
   ...
   local $req_dbg = $req_dbg + 1;
   print STDERR "DOP2Results:: From $deal_t, $$deal_href{NAME}, $$deal_href{OPENER}  { $Debug ge $req_dbg }\n" if $Debug >= $req_dbg;
...
# Next routine fills the deal hash as best it can, when there is only one hand to evaluate.
# In practice this routine only called if there is only a single hand entered.
# In such cases we make that hand the West hand regardless of what the user specified.
# So assume that the Opener is West unless the cmd line said otherwise
 sub Hand2Deal { # deal_href, hand_href
    my $dl  = shift;
    my $hnd = shift ;
    local $req_dbg = $req_dbg + 1;
    my ($i, $ls, $best_fit, $best_DOP, ) ;
    &zero_deal_hash( $dl ) ;  #zero or NULL everything.
    # Now copy over the relevant values from the hand hash.
    $$dl{STR}     = $$hnd{STR};
    $$dl{OPENER}  = 'E' ;
    if ($$hnd{OPEN} eq 'Y' ) { $$dl{OPENER}  = 'W' ; }            # should only call this routine for West to deal.
    $$dl{NAME} = $$hnd{LET};
    print STDERR "HAND2DEAL:: hnd_OPEN=$$hnd{OPEN}, deal_OPENER=$$dl{OPENER} {$Debug ge $req_dbg}\n" if $Debug >= $req_dbg;
    $ls = $$hnd{LEN_ORDER}->[3] ;
    printf STDERR "Hand2Deal:: LongSuit=$ls, $suit_lets[$ls] {$Debug ge $req_dbg}\n" if $Debug >= $req_dbg;
    $$dl{DOP_H}   = $$hnd{DOP_H};
    $$dl{DOP_L}   = $$hnd{DOP_L};
    $$dl{DOP_Sy}  = $$hnd{DOP_Sy};
    $$dl{DOP_D}   = $$hnd{DOP_D};  # mostly for debugging. No use otherwise.
    $$dl{ADJ_TOT} = $$hnd{ADJ_TOT};
    $$dl{DOP_HL}  = $$hnd{DOP_HL};
    $$dl{DOP_HLF} = $$hnd{DOP_HL} ; # the Hf, FN, Mirror, and -Waste terms don't exist for a single hand.


    # Simple stuff done. Now do the NT adjustment for playing in NT with shortness
    # NT DOP seems more complicated than necessary; but the Report routines calculate the final DOP_HLF_NT at the very end.
    # Why I am not sure... Maybe have something to do with the strain reporting ...
    $$dl{DOP_HLF_NT} = $$dl{DOP_HLF};
    $$dl{DOP_NT_DED}->[0] = $$hnd{DOP_NT_DED};   # In this case West is the only hand that has any pts. East is all zero.
    $$dl{DOP_NT_DED}->[1] =  0 ;

    print STDERR "HAND2DEAL:: HLF=$$dl{DOP_HLF}, hnd_NT_DED=$$hnd{DOP_NT_DED}, dl_NT_DED=$$dl{DOP_NT_DED}->[0] {$Debug ge $req_dbg}\n"
         if $Debug >= $req_dbg;

    # Now setup 'fake' DFIT pts array. DFIT**
    # If we are the Opener, Essentially just copy the Dpts to all the suits that are not short.
    # Then use the 'fake' DFIT pts to calculate HLDF[i] for all suits.
    for ($i=0 ; $i < 4 ; $i++) {
       $sptr = $$hnd{SUITS}->[$i];
       if ($$sptr{LEN} >= 3 and $$hnd{OPEN} eq 'Y') { # only opener counts D pts,
          $$dl{DOP_DFIT}->[$i] = $$hnd{DOP_D} ;       # if the suit is longer than 3 it is not a short suit so Dpts OK.
       }
       $$dl{DOP_HLDF_SUIT}->[$i] = $$dl{DOP_HLF} + $$dl{DOP_DFIT}->[$i] ; #-- use the DFIT** added to DOP_HLF
       printf STDERR "HAND2DEAL:: Suit=$i, DOP_D=$$hnd{DOP_D}, Dfit-i=$$dl{DOP_DFIT}->[$i], HLDF-i=$$dl{DOP_HLDF_SUIT}->[$i] {$Debug ge $req_dbg}\n"
            if $Debug >= $req_dbg;
    }

   # Create some frequently used values.
    $$dl{LONG_STRAIN} = $suit_lets[$ls] ;
    $$dl{DOP_HLDFl} = $$dl{DOP_HLDF_SUIT}->[$ls];
    ($best_fit, $best_DOP) = ($ls, $$dl{DOP_HLDFl} ); # for a single hand the longest suit is the best
    $$dl{DOP_HLDFb} = $best_DOP;
    $$dl{BEST_STRAIN} = $suit_lets[$best_fit];

   # Generate the Quick Losers. With only one hand this is much simpler than with two hands.
   ($$dl{FRL},    $$dl{SRL},    @quick_losers  ) = &Quick_Losers_1hand( $hnd, "S") ;
   $ql=&array_sum(\@quick_losers);
   $$dl{QUICK_LOSERS_SUIT} = &array_sum(\@quick_losers);
   ($$dl{NT_FRL}, $$dl{NT_SRL}, @quick_losers  ) = &Quick_Losers_1hand( $hnd, "N") ;
   $ql=&array_sum(\@quick_losers);
   $$dl{QUICK_LOSERS_NT} = &array_sum(\@quick_losers);

   &show_DOP_hash("Hand2Deal {$Debug ge $req_dbg}", $dl ) if $Debug >= $req_dbg;;
   return 1 ;
} # end sub Hand2Deal
