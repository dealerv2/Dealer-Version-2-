...
int knr_calc ( int side ) { /* Tag Number 7 */
   ...
   for (h = 0 ; h < 2 ; h++) {         /* for each hand count up the KnR pts ignoring fit and misfit */
      ...
      UEv.misc_count = 0 ;
      /* The most likely used values will be the complete KnR pts since the plain KnR pts can be got from Dealer direct */
      p_uservals->u.res[0] = KNR_pts[0].knr_rounded_pts + KNR_pts[1].knr_rounded_pts ;
      p_uservals->u.res[1] = KNR_pts[0].knr_rounded_pts;
      p_uservals->u.res[2] = KNR_pts[1].knr_rounded_pts;
      p_uservals->u.res[3] = KnR_side_tot ;  /* The x100 version of the complete KnR Points */
      /* hand[0] details only missing adj for square hand*/
      p_uservals->u.res[4] = KNR_pts[0].knr_honor_pts;
      p_uservals->u.res[5] = KNR_pts[0].knr_short_pts;
      p_uservals->u.res[6] = KNR_pts[0].knr_qual_pts;
      p_uservals->u.res[7] = KNR_pts[0].knr_tot_pts ;
      p_uservals->u.res[8] = KNR_pts[0].knr_dfit;
      p_uservals->u.res[9] = KNR_pts[0].knr_Fn_pts;
      p_uservals->u.res[10]= KNR_pts[0].knr_misfit_adj;
      p_uservals->u.res[11]= KNR_pts[0].knr_body_val ;
      /* hand[1] details only missing adj for square hand */
      p_uservals->u.res[12]= KNR_pts[1].knr_honor_pts;
      p_uservals->u.res[13]= KNR_pts[1].knr_short_pts;
      p_uservals->u.res[14]= KNR_pts[1].knr_qual_pts;
      p_uservals->u.res[15]= KNR_pts[1].knr_tot_pts ;
      p_uservals->u.res[16]= KNR_pts[1].knr_dfit;
      p_uservals->u.res[17]= KNR_pts[1].knr_Fn_pts;
      p_uservals->u.res[18]= KNR_pts[1].knr_misfit_adj;
      p_uservals->u.res[19]= KNR_pts[1].knr_body_val ;
   /* also put main results in standard locations for possible use by other functions */
      UEv.hldf_pts_side    = p_uservals->u.res[0] ;
      UEv.hldf_pts_seat[0] = p_uservals->u.res[1] ;
      UEv.hldf_pts_seat[1] = p_uservals->u.res[2] ;
      UEv.nt_pts_side      = p_uservals->u.res[7] + p_uservals->u.res[15];
      UEv.nt_pts_seat[0]   = p_uservals->u.res[7] ;
      UEv.nt_pts_seat[1]   = p_uservals->u.res[15];
      #ifdef JGMDBG
      if(jgmDebug >= 7 ) {
